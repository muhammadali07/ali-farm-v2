...
import { AppConfig, InvestmentPackage, MarketProduct } from "@/types";
...
export const LandingPage: React.FC<LandingPageProps> = ({ onLogin, lang }) => {
  ...
  const [config, setConfig] = useState<AppConfig>({
    features: { investment: true, qurban: true, marketplace: true },
  });
  const [investmentPackages, setInvestmentPackages] = useState<
    InvestmentPackage[]
  >([]);
  const [qurbanPackages, setQurbanPackages] = useState<QurbanPackage[]>([]);
  ...
  const [qurbanServiceType, setQurbanServiceType] = useState<
    "SHIPPED" | "CUSTODY"
  >("SHIPPED");
  ...
  const PAYMENT_METHODS = [
    {
      ...
      description: isId
        ...
        : "Transfer to BCA/Mandiri account",
      icon: Building2,
    },
    {
      id: "cod",
      name: "COD",
      description: isId
        ? "Bayar ditempat saat barang sampai"
        : "Pay on delivery",
      icon: Banknote,
    },
  ];
  ...
  const BANK_ACCOUNTS = [
    {
      id: "bca",
      name: "Bank BCA",
      accountNumber: "8730456123",
      accountName: "PT Ali Farm Indonesia",
      logo: "🏦",
    },
    {
      id: "mandiri",
      name: "Bank Mandiri",
      accountNumber: "1270008765432",
      accountName: "PT Ali Farm Indonesia",
      logo: "🏛️",
    },
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch config first, use default if fails
        try {
        ...
        // Fetch other data in parallel, ignore errors
        const [investmentData, qurbanData, productData] =
          await Promise.allSettled([
            investmentService.findAllPackages(),
            qurbanService.findAllPackages(),
            marketplaceService.findAll(),
          ]);

        if (investmentData.status === "fulfilled")
          setInvestmentPackages(investmentData.value);
        if (qurbanData.status === "fulfilled")
          setQurbanPackages(qurbanData.value);
        if (productData.status === "fulfilled")
          setMarketProducts(productData.value);
      } catch (error) {
  ...
  const navLinks = [
    ...(config.features.investment
      ? [{ href: "#investments", label: isId ? "Investasi" : "Investments" }]
      : []),
    ...(config.features.qurban
      ? [{ href: "#qurban", label: isId ? "Tabung Qurban" : "Qurban Savings" }]
      : []),
    ...(config.features.marketplace
      ? [{ href: "#marketplace", label: isId ? "Pasar" : "Marketplace" }]
      : []),
  ];
  ...
  const qurbanFee =
    isQurbanOrder && qurbanServiceType === "CUSTODY" ? 50000 : 0;
  const shippingCost = isQurbanOrder
    ? qurbanServiceType === "SHIPPED"
      ? TAXI_FARM.price
      : 0
    : isInvestmentOrder
      ? 0
  ...
  const handleFinalizeWhatsApp = () => {
        ...
        const typeLabel =
          item.type === "Qurban"
            ...
            : item.type === "Investment"
              ? "📈 Investasi"
              : "📦 Produk";
        return `${index + 1}. ${typeLabel}: ${item.name}\n   💰 Rp ${item.price.toLocaleString("id-ID")}`;
      })
      .join("\n");
    ...
    const paymentMethod =
      selectedPayment === "transfer"
        ? `Transfer Bank ${selectedBankData?.name || ""}`
        : "COD (Bayar di Tempat)";
    ...
    if (isQurbanOrder) {
      shippingInfo =
        qurbanServiceType === "SHIPPED"
          ? `🚚 TaxiFarm - Dikirim ke alamat (Rp ${TAXI_FARM.price.toLocaleString("id-ID")})`
          : `🕌 Dititipkan & Disembelih (Rp ${qurbanFee.toLocaleString("id-ID")})`;
    } else if (!isInvestmentOrder) {
      shippingInfo = `🚚 ${selectedCourier.name} - ${selectedCourier.est} (Rp ${selectedCourier.price.toLocaleString("id-ID")})`;
    }

    // Build bank transfer details if applicable
    let bankDetails = "";
    if (selectedPayment === "transfer" && selectedBankData) {
      bankDetails = `
━━━━━━━━━━━━━━━━━━━━
💳 *DETAIL TRANSFER*
Bank: ${selectedBankData.name}
No. Rekening: ${selectedBankData.accountNumber}
Atas Nama: ${selectedBankData.accountName}
Kode Unik: Rp ${uniqueCode.toLocaleString("id-ID")}`;
    }

    const message = `🌿 *PESANAN BARU - ALI FARM*
━━━━━━━━━━━━━━━━━━━━
📋 *ID Pesanan:* ${orderId}
📅 *Tanggal:* ${orderDate}
...
━━━━━━━━━━━━━━━━━━━━
📊 *RINCIAN BIAYA*
Subtotal: Rp ${cartTotal.toLocaleString("id-ID")}${shippingCost > 0 ? `\nOngkir: Rp ${shippingCost.toLocaleString("id-ID")}` : ""}${qurbanFee > 0 ? `\nBiaya Layanan: Rp ${qurbanFee.toLocaleString("id-ID")}` : ""}${transferUniqueCode > 0 ? `\nKode Unik: Rp ${transferUniqueCode.toLocaleString("id-ID")}` : ""}
...
Mohon konfirmasi pesanan ini. Terima kasih! 🙏`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans overflow-x-hidden">
      <nav
        className={`fixed w-full z-[100] transition-all duration-300 ${scrolled ? "bg-white py-3 shadow-md" : "bg-transparent py-6"}`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          >
            ...
            <span
              className={`text-xl font-bold ${scrolled ? "text-slate-900" : "text-white"}`}
            >
              Ali Farm
            </span>
          </div>
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                ...
                className={`text-sm font-medium ${scrolled ? "text-slate-600" : "text-white/90"}`}
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className={`relative p-2 ${scrolled ? "text-slate-700" : "text-white"}`}
            >
              <ShoppingBag size={22} />
              {cart.length > 0 && (
            ...
            <button
              onClick={onLogin}
              className={`px-6 py-2 rounded-full text-sm font-bold ${scrolled ? "bg-slate-900 text-white" : "bg-white text-slate-900"}`}
            >
              {isId ? "Masuk" : "Login"}
            </button>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {cart.length} {isId ? "item" : "items"}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setIsCartOpen(false)}
                className="w-10 h-10 hover:bg-slate-100 rounded-xl flex items-center justify-center transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              {cart.length === 0 ? (
                ...
                cart.map((item, i) => (
                  ...
                  >
                    ...
                    {!item.image && (
                      <div
                        className={`w-20 h-20 rounded-xl flex items-center justify-center ${item.type === "Qurban" ? "bg-emerald-100" : item.type === "Investment" ? "bg-agri-100" : "bg-orange-100"}`}
                      >
                        {item.type === "Qurban" && (
                          <Heart className="w-8 h-8 text-emerald-600" />
                        )}
                        {item.type === "Investment" && (
                          <TrendingUp className="w-8 h-8 text-agri-600" />
                        )}
                        {item.type === "Product" && (
                    ...
                    <div className="flex-1">
                      <span
                        className={`inline-block px-2 py-0.5 text-[10px] font-bold rounded-full mb-1 ${item.type === "Qurban" ? "bg-emerald-100 text-emerald-700" : item.type === "Investment" ? "bg-agri-100 text-agri-700" : "bg-orange-100 text-orange-700"}`}
                      >
                        {item.type === "Qurban"
                          ? "TABUNG QURBAN"
                          : item.type === "Investment"
                      ...
                      <p className="text-agri-600 font-bold">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-3xl relative overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 border-b bg-slate-50">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() => setIsCheckoutOpen(false)}
                  className="w-10 h-10 hover:bg-slate-200 rounded-xl flex items-center justify-center transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3].map((step) => (
                  <React.Fragment key={step}>
                    <div
                      className={`flex items-center gap-2 ${checkoutStep >= step ? "text-agri-600" : "text-slate-300"}`}
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${checkoutStep >= step ? "bg-agri-600 text-white" : "bg-slate-200 text-slate-400"}`}
                      >
                        {checkoutStep > step ? (
                    ...
                    {step < 3 && (
                      <div
                        className={`flex-1 h-1 rounded ${checkoutStep > step ? "bg-agri-600" : "bg-slate-200"}`}
                      ></div>
                    )}
                  </React.Fragment>
                ))}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">
                                a.n {selectedBankData.accountName}
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5">
                        ...
                        <div className="bg-white rounded-xl p-4">
                          ...
                          <div className="flex items-center gap-2 text-sm text-emerald-700 bg-emerald-50 p-2 rounded-lg">
                            ...
                            <span>
                              {isId
                                ? `Termasuk kode unik Rp ${uniqueCode}`
                                : `Includes unique code Rp ${uniqueCode}`}
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          <div>
                            ...
                            <div className="space-y-3">
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <Truck
                                    className={`w-6 h-6 ${qurbanServiceType === "SHIPPED" ? "text-emerald-600" : "text-slate-400"}`}
                                  />
                                  <div className="flex-1">
                                    <p className="font-bold text-slate-900">
                                      {isId
                                    ...
                                    <p className="text-sm text-slate-500">
                                      {isId
                                        ? "Domba diantar hidup ke lokasi Anda"
                                        : "Live sheep delivered to your location"}
                                    </p>
                                  </div>
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <HandHeart
                                    className={`w-6 h-6 ${qurbanServiceType === "CUSTODY" ? "text-emerald-600" : "text-slate-400"}`}
                                  />
                                  <div className="flex-1">
                                    <p className="font-bold text-slate-900">
                                      {isId
                                        ? "Dititipkan & Disembelih"
                                        : "Custody & Slaughter"}
                                    </p>
                                    <p className="text-sm text-slate-500">
                                      {isId
                                        ? "Kami yang urus pemotongan & distribusi"
                                        : "We handle slaughter & distribution"}
                                    </p>
                                  </div>
                                  <span className="text-sm font-bold text-emerald-600">
                                    Rp 50.000
                          ...
                          <div className="space-y-3">
                            {PAYMENT_METHODS.map((m) => (
                              ...
                              >
                                <div className="flex items-center gap-4">
                                  <div
                                    className={`w-12 h-12 rounded-xl flex items-center justify-center ${selectedPayment === m.id ? "bg-agri-100" : "bg-slate-100"}`}
                                  >
                                    <m.icon
                                      className={`w-6 h-6 ${selectedPayment === m.id ? "text-agri-600" : "text-slate-400"}`}
                                    />
                                  </div>
                                  <div className="flex-1">
                                    <p className="font-bold text-slate-900">
                                  ...
                                  <div
                                    className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedPayment === m.id ? "border-agri-600 bg-agri-600" : "border-slate-300"}`}
                                  >
                            ...
                            <div className="space-y-3">
                              {BANK_ACCOUNTS.map((bank) => (
                                ...
                                >
                                  <div className="flex items-center gap-4">
                                    <div
                                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl ${selectedBank === bank.id ? "bg-agri-100" : "bg-slate-100"}`}
                                    >
                                      {bank.logo}
                                    </div>
                                    <div className="flex-1">
                                    ...
                                    <div
                                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedBank === bank.id ? "border-agri-600 bg-agri-600" : "border-slate-300"}`}
                                    >
                    ...
                    <div className="bg-slate-50 rounded-2xl p-5 sticky top-0">
                      ...
                      <div className="border-t border-slate-200 pt-4 space-y-2">
                        ...
                        {qurbanFee > 0 && (
                          <div className="flex justify-between text-sm">
                            <span className="text-slate-600">
                              {isId ? "Biaya Layanan" : "Service Fee"}
                            </span>
                            <span className="font-medium">
                              Rp {qurbanFee.toLocaleString()}
                            </span>
                          </div>
                        )}
                        {selectedPayment === "transfer" &&
                          transferUniqueCode > 0 && (
                            <div className="flex justify-between text-sm">
                              <span className="text-slate-600">
                                {isId ? "Kode Unik" : "Unique Code"}
                              </span>
                              <span className="font-medium text-yellow-600">
                                Rp {transferUniqueCode.toLocaleString()}
                              </span>
                            </div>
                          )}
                        <div className="flex justify-between pt-2 border-t border-slate-200">
                          ...
                          <span className="font-bold text-lg text-agri-600">
                            Rp {finalTotal.toLocaleString()}
                          </span>
                        </div>
                      </div>
            ...
            <div className="p-6 border-t bg-white">
              {checkoutStep === 3 ? (
                <button
                  ...
                  className="w-full py-4 bg-green-600 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-green-700 transition-colors"
                >
      ...
      <section className="h-screen relative flex items-center justify-center">
        ...
        <div className="relative z-10 text-center text-white px-4">
          <h1 className="text-5xl md:text-7xl font-bold mb-6">
            Investasi Ternak Digital
          </h1>
          <p className="text-xl text-slate-300 max-w-2xl mx-auto mb-10">
            Masa depan peternakan transparan dengan bagi hasil syariah dan
            pantauan CCTV 24/7.
          </p>
          <button
            onClick={(e) => scrollToSection(e, "#investments")}
            className="px-10 py-4 bg-agri-600 rounded-full font-bold text-lg shadow-xl hover:scale-105 transition-all"
          >
            Mulai Sekarang
          </button>
        </div>
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              ...
              <div>
                ...
                <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">
                  {isId
                    ? "Investasi Domba dengan Bagi Hasil Syariah"
                    : "Sheep Investment with Sharia Profit Sharing"}
                </h2>
                ...
                <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-slate-700 font-medium">
                      {isId ? "Akad Syariah" : "Sharia Contract"}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-agri-100 rounded-xl flex items-center justify-center">
                  ...
                  {investmentPackages.length > 0 ? (
                    <>
                      <h3 className="text-2xl font-bold mb-2">
                        {investmentPackages[0].name}
                      </h3>
                      <p className="text-white/80 mb-6">
                        {investmentPackages[0].description}
                      </p>
                      <div className="mb-6">
                        <span className="text-4xl font-bold">
                          Rp{" "}
                          {investmentPackages[0].pricePerUnit.toLocaleString()}
                        </span>
                      ...
                      <div className="space-y-3 mb-8 text-white/90">
                        <div className="flex items-center gap-3">
                          ...
                          <span>
                            {isId ? "Durasi kontrak" : "Contract duration"}:{" "}
                            {investmentPackages[0].durationMonths}{" "}
                            {isId ? "Bulan" : "Months"}
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          <CheckCircle className="w-5 h-5 text-green-300" />
                          <span>
                            {isId ? "Estimasi ROI" : "Est. ROI"}:{" "}
                            {investmentPackages[0].estimatedRoi}%
                          </span>
                        </div>
                        <div className="flex items-center gap-3">
                          <CheckCircle className="w-5 h-5 text-green-300" />
                      ...
                      <button
                        onClick={() =>
                          addToCart({
                            id: investmentPackages[0].id,
                            name: investmentPackages[0].name,
                            price: investmentPackages[0].pricePerUnit,
                            type: "Investment",
                          })
                        }
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-white/70">
                        {isId ? "Memuat paket..." : "Loading packages..."}
                      </p>
                    </div>
                  ) : (
                    <div className="text-center py-8">
                      ...
                      <p className="text-white/50 text-sm">
                        {isId
                          ? "Paket investasi sedang disiapkan"
                          : "Investment packages are being prepared"}
                      </p>
                    </div>
                  )}
                </div>
      ...
      {config.features.qurban && (
        <section
          id="qurban"
          className="py-24 bg-gradient-to-br from-emerald-600 via-emerald-700 to-teal-800 relative overflow-hidden"
        >
          ...
          <div className="max-w-7xl mx-auto px-6 relative z-10">
            ...
            <div className="grid md:grid-cols-3 gap-8 mb-16">
              {qurbanPackages.length > 0 ? (
                qurbanPackages.map((pkg, index) => (
                  <div
                    key={pkg.id}
                    className={`relative bg-white rounded-3xl overflow-hidden shadow-2xl transition-all hover:scale-105 ${index === 1 ? "md:-translate-y-4 ring-4 ring-yellow-400" : ""}`}
                  >
                    {index === 1 && (
                    ...
                    <div className="aspect-[4/3] overflow-hidden">
                      <img
                        src={
                          pkg.imageUrl ||
                          "https://images.unsplash.com/photo-1511117833895-4b473c0b85d6?w=400"
                        }
                        alt={pkg.name}
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          ...
                          <p className="text-xs text-slate-400">
                            {isId ? "atau cicil" : "or installment"} Rp{" "}
                            {Math.ceil(pkg.price / 12).toLocaleString()}/
                            {isId ? "bln" : "mo"}
                          </p>
                        </div>
                      </div>
                ...
                <div className="col-span-3 text-center py-12">
                  ...
                  <p className="text-white/70">
                    {isId
                      ? "Memuat paket qurban..."
                      : "Loading qurban packages..."}
                  </p>
                </div>
              ) : (
                <div className="col-span-3 text-center py-12">
                  <Heart className="w-12 h-12 mx-auto mb-4 text-white/50" />
                  <p className="text-white/70 mb-2">
                    {isId ? "Segera Hadir" : "Coming Soon"}
                  </p>
                  <p className="text-white/50 text-sm">
                    {isId
                      ? "Paket qurban sedang disiapkan"
                      : "Qurban packages are being prepared"}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="grid md:grid-cols-4 gap-6">
              <div className="bg-white/10 backdrop-blur p-5 rounded-2xl text-white text-center">
                ...
                <h4 className="font-bold text-lg mb-1">
                  {isId ? "Cicilan Fleksibel" : "Flexible Installment"}
                </h4>
                <p className="text-emerald-200 text-sm">
                  {isId ? "Mulai Rp 100.000/bulan" : "From Rp 100,000/month"}
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur p-5 rounded-2xl text-white text-center">
                <ShieldCheck className="w-10 h-10 text-yellow-300 mx-auto mb-3" />
                <h4 className="font-bold text-lg mb-1">
                  {isId ? "Dijamin Syariah" : "Sharia Guaranteed"}
                </h4>
                <p className="text-emerald-200 text-sm">
                  {isId ? "Akad jelas & transparan" : "Clear & transparent"}
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur p-5 rounded-2xl text-white text-center">
                ...
                <p className="text-emerald-200 text-sm">
                  {isId ? "Siap saat Idul Adha" : "Ready for Eid al-Adha"}
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur p-5 rounded-2xl text-white text-center">
                <Truck className="w-10 h-10 text-yellow-300 mx-auto mb-3" />
                <h4 className="font-bold text-lg mb-1">
                  {isId ? "Gratis Antar" : "Free Delivery"}
                </h4>
                <p className="text-emerald-200 text-sm">
                  {isId ? "Ke lokasi pemotongan" : "To slaughter location"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="text-center mb-16">
              ...
              <p className="text-slate-500 max-w-2xl mx-auto">
                {isId
                  ? "Dapatkan pakan berkualitas, domba bibit unggulan, dan kebutuhan peternakan lainnya."
                  : "Get quality feed, superior breeding sheep, and other farming needs."}
              </p>
            </div>
            <div className="grid md:grid-cols-3 gap-8">
              {marketProducts.length > 0 ? (
                marketProducts.map((product) => (
                  <div
                    key={product.id}
                    className="group bg-slate-50 rounded-3xl overflow-hidden hover:shadow-xl transition-all"
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xl font-bold text-agri-600">
                ...
                <div className="col-span-3 text-center py-12">
                  ...
                  <p className="text-slate-500">
                    {isId ? "Memuat produk..." : "Loading products..."}
                  </p>
                </div>
              ) : (
                <div className="col-span-3 text-center py-12">
                  ...
                  <p className="text-slate-400 text-sm">
                    {isId
                      ? "Produk marketplace sedang disiapkan"
                      : "Marketplace products are being prepared"}
                  </p>
                </div>
              )}
            </div>
      ...
      <footer className="py-12 bg-slate-50 border-t text-center text-slate-400">
        <p>© 2023 Ali Farm. Smart Sheep Farming & Investment Platform.</p>
      </footer>
    </div>
  );
};
