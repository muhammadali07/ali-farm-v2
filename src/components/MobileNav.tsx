...
export const MobileNav: React.FC = () => {
  ...
  // Get current path to determine 'active' status for buttons
  ...
  const productItems = [
    { id: 'sheep', path: '/sheep', label: 'Sheep', icon: Sprout, color: 'text-green-600 bg-green-50' },
    { id: 'investments', path: '/investments', label: 'Invest', icon: Users, color: 'text-blue-600 bg-blue-50' },
    { id: 'marketplace', path: '/marketplace', label: 'Shop', icon: ShoppingBag, color: 'text-orange-600 bg-orange-50' },
  ];
  ...
  return (
    <>
      ...
      <div
        ref={menuRef}
        className={`fixed bottom-24 left-1/2 -translate-x-1/2 z-50 md:hidden transition-all duration-300 ${isProductMenuOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95 pointer-events-none'
          }`}
      >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 px-8 py-3 pb-safe md:hidden z-50 flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
        ...
        <button
          ...
          className={`relative -top-6 bg-slate-900 text-white p-4 rounded-full shadow-lg shadow-slate-900/30 transition-transform duration-200 ${isProductMenuOpen ? 'scale-110 bg-agri-600 ring-4 ring-agri-100' : 'active:scale-95'
            }`}
        >
